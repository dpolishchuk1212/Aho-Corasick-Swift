...
public class IntervalNode<T: Interval> {
  ...
  public func findOverlaps(interval: T) -> [T] {
    ...
    if point < interval.start {
      overlaps += overlap(
        interval: interval, newOverlaps: findOverlappingRanges(node: right, interval: interval))
      overlaps += overlap(
        interval: interval, newOverlaps: checkForOverlaps(in: interval, toThe: .right))
    } else if point > interval.end {
      overlaps += overlap(
        interval: interval, newOverlaps: findOverlappingRanges(node: left, interval: interval))
      overlaps += overlap(
        interval: interval, newOverlaps: checkForOverlaps(in: interval, toThe: .left))
    } else {
