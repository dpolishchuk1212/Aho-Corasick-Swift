...
public struct Trie {
  ...
  public func tokenize(text: String) -> [Token] {
    ...
    for emit in collectedEmits {
      if emit.start - lastCollectedPosition > 1 {
        tokens.append(
          createFragment(emit, text: text, lastCollectedPosition: lastCollectedPosition))
      }
  ...
  private func removePartialMatches(searchText: String, collectedEmits: [Emit]) -> [Emit] {
    ...
    let nonPartialMatches = collectedEmits.filter {
      !isPartialMatch(searchText: searchText, emit: $0)
    }
